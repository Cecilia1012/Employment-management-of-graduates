...
include("../html/student_edit_chuangye.html");
...
{
	//点击确认按钮
	if(isset($_POST["chuangyeEditBtn"]))
	{
		include("conn.php");
		$sql="SELECT ID,company,job,address,phonenum FROM business WHERE isuse=1 AND ID='$editId'";
		...
		else
		{
			...
			if($test!=0)
			{
				...
				//修改创业表中的信息
				$sql2="UPDATE business SET company='$companyEditT',job='$jobEditT',address='$companyAddressEditT',phonenum='companyPhoneEditT'  WHERE ID='$editId' AND isuse=1";
			...
			else
			{
				...
				$sql2="INSERT INTO business (ID,company,job,address,phonenum,isuse) VALUES('$editId','$companyEditT','$jobEditT','$companyAddressEditT','$companyPhoneEditT',1)";
				$rs2 = mysqli_query($conn,$sql2);
				if(!$rs2)
				{
					printf("Error: %s\n", mysqli_error($conn));
	...
	//点击返回按钮
	else if(isset($_POST["chuangyeBackEditBtn"]))
	{
		header("Refresh:0.1;url=student_formation.php");
	}
}
